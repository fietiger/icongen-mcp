...
class RunCmdService:
    ...
    def run_command(self, command: str, timeout: int = 30, working_directory: Optional[str] = None) -> str:
        """
        异步运行命令
    ...
    def _execute_command(self, token: str, command: str, timeout: int, working_directory: Optional[str]):
        """
        在单独线程中执行命令
        """
        try:
